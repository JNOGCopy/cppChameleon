...
void *unCompressData(const char *data, size_t compressedSize,
                     size_t &originalSize) {
  // Decompress data
  ...
  // Decompress data
  size_t result =
      ZSTD_decompress(decompressedData, originalSize, data, compressedSize);
...
void sendPacketAndCompress(ENetPeer *to, Packet p, const char *data,
                           size_t size, bool reliable, int channel) {
...
void sendPacket(ENetPeer *to, uint32_t header, void *data, size_t size,
                bool reliable, int channel) {
  Packet packet;
...
void sendPacket(ENetPeer *to, uint32_t header, std::uint64_t cid, void *data,
                size_t size, bool reliable, int channel) {
  Packet packet;
  packet.header = header;
  packet.cid = cid;
