...
void setConnectionState(ClientNetworking &service,
                        ClientNetworking::ConnectionState state,
                        std::string status) {
...
ClientNetworking::RoundPhase decodeRoundPhase(std::uint32_t roundPhase) {
  switch (roundPhase) {
  case roundPhaseHiderHide:
    return ClientNetworking::RoundPhase::HiderHide;
  case roundPhaseHunterSearch:
    return ClientNetworking::RoundPhase::HunterSearch;
  default:
    return ClientNetworking::RoundPhase::Lobby;
  }
}
} // namespace
...
bool ClientNetworking::connectToServer(const char *serverAddress) {
  ...
  connectedServerAddress =
      (serverAddress && serverAddress[0]) ? serverAddress : "localhost";
  ...
  if (!client) {
    setConnectionState(*this, ConnectionState::Failed,
  ...
  if (!serverPeer) {
    destroyClientHost(*this);
    setConnectionState(*this, ConnectionState::Failed,
                       "Failed to create ENet peer.");
    return false;
  }

  setConnectionState(*this, ConnectionState::Connecting,
                     "Connecting to " + connectedServerAddress + ":" +
                         std::to_string(NET_PULSE_SERVER_PORT) + "...");
  ...
  while (std::chrono::steady_clock::now() < deadline) {
    const auto remainingMilliseconds =
        std::chrono::duration_cast<std::chrono::milliseconds>(
            ...
            .count();
    const int waitTime =
        (std::max)(1, (std::min)(CONNECT_POLL_SLICE_MS,
    ...
    if (serviceResult < 0) {
      destroyClientHost(*this);
      setConnectionState(*this, ConnectionState::Failed,
                         "Failed while waiting for server connection.");
      return false;
    }
    ...
    case ENET_EVENT_TYPE_CONNECT: {
      setConnectionState(*this, ConnectionState::Connected,
                         "Connected. Waiting for player data...");
      break;
    }
    ...
    case ENET_EVENT_TYPE_DISCONNECT: {
      ...
      destroyClientHost(*this);
      setConnectionState(*this, ConnectionState::Failed,
                         "Disconnected before receiving player data.");
      return false;
    }
...
bool ClientNetworking::sendPlayerState(
    const Packet_PlayerStateUpdate &playerState, bool reliable) {
  if (!client || !serverPeer || connectionState != ConnectionState::Connected ||
      localCID == 0) {
    return false;
  }

  sendPacket(serverPeer, headerPlayerStateUpdate, localCID,
             (void *)&playerState, sizeof(playerState), reliable,
             CHANNEL_GAMEPLAY);
...
bool ClientNetworking::sendHunterHitPlayer(std::uint64_t targetCID) {
  ...
  sendPacket(serverPeer, headerHunterHitPlayer, localCID, (void *)&hitPacket,
             sizeof(hitPacket), true, CHANNEL_CONNECTIONS);

  return true;
}

bool ClientNetworking::sendPaintTextureUpdate(
    int meshIndex, glm::ivec2 size, int quality,
  ...
  sendPacketAndCompress(serverPeer, packet,
                        reinterpret_cast<const char *>(payload.data()),
                        payload.size(), true, CHANNEL_PAINTING);
...
void ClientNetworking::update(float deltaTime) {
  ...
  while ((enet_host_service(client, &event, waitTime) > 0) ||
         (waitTime = 0, tries-- > 0)) {
    waitTime = 0;
    ...
    case ENET_EVENT_TYPE_DISCONNECT: {
      ...
      remotePaintUpdates.clear();
      foundHiderIDs.clear();
      setConnectionState(*this, ConnectionState::Disconnected,
  ...
  if (gameActive && roundPhase != RoundPhase::Lobby && deltaTime > 0.0f) {
    roundTimerRemainingSeconds =
        (std::max)(0.0f, roundTimerRemainingSeconds - deltaTime);
  }
}
...
void ClientNetworking::receiveDataFromServer(ENetEvent &event) {
  ...
  if (packet.isCompressed()) {
    ...
    ownedDecompressedData = static_cast<char *>(
        unCompressData(packetData, packetDataSize, originalSize));
    if (!ownedDecompressedData) {
      setConnectionState(*this, ConnectionState::Failed,
                         "Failed to decompress packet from server.");
      return;
    }
  ...
  case headerReceiveCIDAndData: {
    if (payloadSize >= sizeof(Packet_ReceiveCIDAndData)) {
      const auto &receivedPacket =
          *reinterpret_cast<const Packet_ReceiveCIDAndData *>(payload);
      localCID = receivedPacket.yourCID;
      receivedPlayerData = true;
      setConnectionState(*this, ConnectionState::Connected,
                         "Received player data from server.");
    }
    break;
  }
  ...
  case headerGameStateUpdate: {
    if (payloadSize >= sizeof(Packet_GameStateUpdate)) {
      ...
      const auto &receivedPacket =
          *reinterpret_cast<const Packet_GameStateUpdate *>(payload);
      ...
      totalHiderCount = gameActive ? receivedPacket.totalHiderCount : 0u;
      roundPhase = gameActive ? decodeRoundPhase(receivedPacket.roundPhase)
                              : RoundPhase::Lobby;
      roundTimerRemainingSeconds =
          gameActive ? static_cast<float>(receivedPacket.timerSeconds) : 0.0f;

      const bool enteredHidePhase =
          roundPhase == RoundPhase::HiderHide &&
          (!wasGameActive || previousRoundPhase != RoundPhase::HiderHide);
      if (!gameActive || mapChanged || enteredHidePhase) {
        localPlayerFound = false;
        roundResult = RoundResult::None;
        foundHiderIDs.clear();
      } else {
      ...
      if (!gameActive) {
        ...
        lastStatus =
            "Hide time started. Hunter CID: " + std::to_string(hunterCID) + ".";
      } else if (roundPhase == RoundPhase::HunterSearch) {
        lastStatus =
            "Hunter search started. Hunter CID: " + std::to_string(hunterCID) +
            ".";
      } else {
        lastStatus =
            "Game started. Hunter CID: " + std::to_string(hunterCID) + ".";
      }
    }
    break;
  }
  ...
  case headerPlayerFoundNotification: {
    if (payloadSize >= sizeof(Packet_PlayerFoundNotification)) {
      const auto &receivedPacket =
          *reinterpret_cast<const Packet_PlayerFoundNotification *>(payload);
      if (receivedPacket.targetCID != 0 &&
          !isPlayerFound(receivedPacket.targetCID)) {
  ...
  case headerRoundResult: {
    if (payloadSize >= sizeof(Packet_RoundResult)) {
      const auto &receivedPacket =
          *reinterpret_cast<const Packet_RoundResult *>(payload);
  ...
  case headerPlayerStateUpdate: {
    if (payloadSize >= sizeof(Packet_PlayerStateUpdate) && packet.cid != 0 &&
        packet.cid != localCID) {
      const auto &receivedPacket =
          *reinterpret_cast<const Packet_PlayerStateUpdate *>(payload);
  ...
  case headerPlayerPaintTextureUpdate: {
    if (packet.cid != 0 && packet.cid != localCID &&
        payloadSize >= sizeof(Packet_PlayerPaintTextureUpdate)) {
      const auto &receivedHeader =
          *reinterpret_cast<const Packet_PlayerPaintTextureUpdate *>(payload);
      const size_t pixelBytes =
          payloadSize - sizeof(Packet_PlayerPaintTextureUpdate);
      if (receivedHeader.meshIndex >= 0 && receivedHeader.size.x > 0 &&
          receivedHeader.size.y > 0 &&
          receivedHeader.pixelDataSize == pixelBytes) {
        auto &remotePaint =
            remotePaintUpdates[packet.cid][receivedHeader.meshIndex];
        ...
        if (pixelBytes > 0) {
          std::memcpy(remotePaint.pixels.data(),
                      reinterpret_cast<const unsigned char *>(payload) +
                          sizeof(Packet_PlayerPaintTextureUpdate),
                      pixelBytes);
        }
      }
    }
  ...
  case headerClientDisconnected: {
    if (payloadSize >= sizeof(Packet_ClientDisconnected)) {
      const auto &receivedPacket =
          *reinterpret_cast<const Packet_ClientDisconnected *>(payload);
      remotePlayers.erase(receivedPacket.cid);
      remotePaintUpdates.erase(receivedPacket.cid);
      foundHiderIDs.erase(std::remove(foundHiderIDs.begin(),
                                      foundHiderIDs.end(), receivedPacket.cid),
                          foundHiderIDs.end());
    }
    break;
  }
  ...
  default:
    lastStatus =
        "Received unknown packet header " + std::to_string(header) + ".";
    break;
  }
...
bool ClientNetworking::isLocalHunter() const { return isHunter(localCID); }

bool ClientNetworking::isPlayerFound(std::uint64_t cid) const {
  return std::find(foundHiderIDs.begin(), foundHiderIDs.end(), cid) !=
         foundHiderIDs.end();
}
